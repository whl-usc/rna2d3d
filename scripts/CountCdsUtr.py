...
def bam2bed(input_bam, output_bed):
    ...
    with open(temp_unsorted, "w") as out:
        subprocess.run(["bedtools", "bamtobed", "-i", input_bam, "-split"], stdout=out, check=True)

    with open(output_bed, "w") as sorted_out:
        subprocess.run(["sort", "-k1,1", "-k2,2n", temp_unsorted], stdout=sorted_out, check=True)
...
def write_output(prefix, genes, biotypes, counts, total_reads, *dicts):
    ...
    def calc_rpkm(count, length):
        return 0.01 if count == 0 or length == 0 else count / (length / 1000 * total_reads / 1_000_000)

    def calc_rpm(count):
        return 0.01 if count == 0 else count / total_reads * 1_000_000

    with open(rpkm_file, "w") as f:
        f.write("Gene\tBiotype\tgene_RPKM\tCDS_RPKM\tIntron_RPKM\t5UTR_RPKM\t3UTR_RPKM\n")
        for gene in sorted(genes):
            biotype = biotypes.get(gene, "NA")
            line = f"{gene}\t{biotype}\t" + "\t".join([
    ...
    with open(count_file, "w") as f:
        f.write("Gene\tBiotype\tgene_Reads\tCDS_overlaps\tIntron_overlaps\t5UTR_overlaps\t3UTR_overlaps\n")
        for gene in sorted(genes):
            biotype = biotypes.get(gene, "NA")
            line = f"{gene}\t{biotype}\t" + "\t".join([
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Counts reads in CDS, 5'UTR, and 3'UTR from BAM using BED.")
    ...
    VALID_BIOTYPES = {
        "none", "lncRNA", "miRNA", "misc_RNA", "mt-tRNA", "protein_coding",
        "RNR", "rRNA", "scaRNA", "scRNA", "SNORA", "SNORD", "snRNA", "tRNA"
        }
    parser.add_argument(
        "exclude",
