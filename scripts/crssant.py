...
Updated: WHL- 03/2024. Added benchmarking and formatting fixes. 

List of functions: 6 sections, roughly in logical order.
...
def getreads(alignfile):
    """
    Function to build a dictionary of all input reads. 

    input:  gap1.sam and trans.sam from gapanalysis.py and gapfilter.py 
            The two types of input are automatically detected. 
    ...
    for line in f:
        if line[0]=='@': header.append(line.strip()); continue
        QNAME = line.split('\t')[0]
        if QNAME not in readsdict: readsdict[QNAME]=[]; readcount+=1
        readsdict[QNAME].append(line.strip()); aligncount+=1
...
def getgenes(genesfile):
    """
    Makes a sorted list for genes from bed input.
    
    Input BED6: chrom chromStart chromEnd name score strand (see bamget.py)
    
    returns genesdict:  (chr,strand,pos): [names] 
                        (Genes may overlap, pos in 10nt intvls)
    """
...
def line2info(line, genesdict):
    """
    Converts 1 chimeric alignment line into a list of information.
    """
    align=line.split('\t')
...
def getalign(readsdict, genesdict):
    """
    Function to build a dictionary of noncontinous alignments. Requires
    itertools.product, line2info(), exports alignments outside of gene 
    models to another sam file.

    Build a dictionary of genepairs and alignments: genealigndict
    (gene1,gene2):[[alignid,info1,info2]...]. info:chrom,strand,start,end,line
    
    Inputs: 
        readsdict: {readid:[alignments (str)], ...}
        genesdict: {(chrom,strand,pos): [name ...] ...}

    aligndict:  format for each item (line1, line 2 identical chimera):
    ...
    for key in aligndict:
        ...
        if not genes1 or not genes2: continue #Export alignments later
...
def get_overlaps(inds_1, inds_2):
    """
    Calculates overlaps between two reads and the distance spanned by them.
    Reads are each assumed to comprise two arms; left and right arm.
    
    input:  inds_1 and inds_2
...
def get_overlap_ratios(inds_1, inds_2):
    """
    Function to calculate overlap ratio between two reads.
...
def process_cigar(cigar_str):
    """
    Function to process CIGAR string from sequencing reads.

    input:  cigar_str (str; CIGAR string)

    returns ops, lens (list, list)
    """
    # Parse CIGAR string into operations (ops) and operation lengths (lens)
...
def subsample(genealign, covlimit):
    """
    Takes a subsample of the input reads once it exceeds covlimit. 
    
    input:  genealign:  [[alignid,info1,info2]...]
            info:       chrom,strand,start,end,line
            covlimit    maximum number of reads to parse (int)
    
    returns genealignlimit, geneextra
    """
    ...
    genealignlimit = genealign #First set limited list to all alignments
...
def graph_align(genealign, t): 
    ...
    input genealign is one item from the genealigndict dictionary. Only
    considers subsampled reads for graphing.

    input:  info:chrom,strand,start,end,line
            alignids: np array. Read IDs
            genealign:  list of alignments 
                        (gene1,gene2):[[alignid,info1,info2]...]
            t: float. Overlap threshold, default (0.1) cliques, (0.5) spectral
...
def get_spectral(graph, n=10, t=5): #This function has bugs that are not fixed. 
    """
    ...
    for subgraph in subgraphs:
        ...
        if len(subgraph) > 1:
            ...
            w, v = sp.linalg.eigh(L, D, type=1)  # Since L is always symmetric
    ...
    return align_dg_dict #Assignment of alignment IDs to DG numbers.
...
def get_dgs(align_dg_dict):
    ...
    Function that creates inverse dictionary of align_dg_dict
        align_dg_dict: Dictionary of alignments and clustering DG assignments
    
    returns dg_align_dict: dict, k=dg_id, v=[alignids]
    
    NOTE: align_dg_dict comes from get_spectral(graph) or get_cliques(graph)
    """
...
def addextra(dg_align_dict, genealign, geneextra, t_o):
    """
    Function to get the medians of each DG terminal, passes remaining reads 
    into dg_align_dict.

    input:  dg_align_dict: k=dg_id; v=[alignids]
            genealign: [[alignid,info1,info2]...]. info:chrom,strand,start,end,line
    
    returns dg_align_dict: dict, k=dg_id, v=[alignids]
    
    Requires: get_overlaps, get_overlap_ratios
    """    
...
def filter_dgs(dg_align_dict, genealign):
    """
    Function to filter out invalid DGs. DGs whose alignments are all identical 
    are eliminated. Compares (chr1,strand1,start1,end1,chr2,strand2,start2,end2)
    DGs with fewer than two alignments are also eliminated.
    
    input:  
        dg_align_dict: Dictionary of DGs and their alignments
        genealign: (gene1,gene2):[[alignid,info1,info2]...]. 
        info:chrom,strand,start,end,line
...
def create_stats(dg_align_dict, genealign, covdict):
    ...
    input:
        ...
        covdict: (chrom,strand,pos):maxvalue 
    
    returns dg_stats_dict
    """
    ng_ind = 0
    ...
    for (dg, alignids) in dg_align_dict.items():
        ...
        # To visualize alignments properly in IGV, set "Sort alignments by tag 
        # DG" and "Group alignments by tag NG"
...
def writesam(outsam,genealigndict,dg_filtered_dict,dg_stats_dict,genepair):
    """
    Function that writes the output SAM file, which includes only reads that
    contributed to DG groups (i.e., if DG had only one read, which does not
    pass the DG filter and the read is not output).

    input:
        genealigndict: 
            (gene1,gene2):[[alignid,info1,info2]...]. 
            info:chrom,strand,start,end,line
        dg_filtered_dict: Dict DG and alignments {dg_id:align_id}
        dg_stats_dict: Dict DG metadata, arm_inds,covfraction,num_align,NG
        genepair: tuple (gene1,gene2). Intramolecular structure if gene1==gene2
        pseudocode: for each dg in dg_filtered_dict, get alignments from
                    aligndict and ng information from dg_stats_dict, then write
                    alignments to outsam
    """
    readsdict = {}
    f= open(outsam, 'a')
...
def writedg(outdg,dg_stats_dict,genepair):
    """
    Writes assembled DGs to bedpe format as output. Additional bedpe can be 
    converted to bed12 format to visualize arcs. 
...
def parse_args():
    ...
    parser.add_argument('bedgraphs', help='Path to genome coverge (bedgraph)'
                        'Comma separated list of files for the + and - strands')
    parser.add_argument('-out', help='Path of output')
    parser.add_argument('-cluster', help='"cliques" or "spectral" (default)')
    parser.add_argument('-n', help='Number of threads. Defaults to 8')
...
def main():
    args = parse_args()
    # 1) Read bed references and sequencing alignments.
